...
void Game::Init() {
	// load shaders
	ResourceManager::LoadShader("./shader/sprite.vert", "./shader/sprite.frag",
								nullptr, "sprite");
	// configure shaders
	glm::mat4 projection =
	...
	ResourceManager::LoadTexture("./texture/awesomeface.png", true,
								 "plane_basic");
	ResourceManager::LoadTexture("./texture/stars.png", true, "background");
	// 设置缩放
	...
	this->Commands[GLFW_KEY_A] = &moveLeftCommand;
	this->Commands[GLFW_KEY_D] = &moveRightCommand;
	this->Commands[GLFW_KEY_W] = &moveTopCommand;
	this->Commands[GLFW_KEY_S] = &moveBottomCommand;
	// load Player
	auto t = ResourceManager::GetTexture("plane_basic");
	glm::vec2 playerSize = glm::vec2(t.Width, t.Height) * PlaneSpriteScale;
	glm::vec2 playerPos =
		glm::vec2(static_cast<float>(this->Width) / 2.0F - playerSize.x / 2.0F,
				  static_cast<float>(this->Height) - playerSize.y);
	this->Player = new PlaneObject(playerPos, playerSize, t, PLAYER_COLOR);
	// load Levels
	...
	this->CurrentLevel = 0;
	std::cerr << "level count: " << this->Levels.size() << "\n";
...
void Game::Update(float dt) {
	this->Player->update(this->Player, dt, this->Width, this->Height);
...
auto Game::InputHandle(unsigned char *keys) -> std::vector<Command *> {
	std::vector<Command *> res;
	for (int i = 0; i < 1024; ++i) {
		if (keys[i] != 0U) {
			res.emplace_back(this->Commands[i]);
...
void Game::ProcessInput(float dt) {
	...
	static unsigned char aiInput[1024] = {};
	for (auto *i : Enemies) {
		memset(aiInput, 0, sizeof(unsigned char) * 1024);
		i->Ai->Update(i, Player, aiInput, this->Height, this->Width,
					  this->rnd);
		// if (aiInput[GLFW_KEY_W] != 0U) {
		// 	std::cerr << "ai input W \n";
		// }
		// if (aiInput[GLFW_KEY_A] != 0U) {
		// 	std::cerr << "ai input A \n";
		// }
		// if (aiInput[GLFW_KEY_S] != 0U) {
		// 	std::cerr << "ai input S \n";
		// }
		// if (aiInput[GLFW_KEY_D] != 0U) {
		// 	std::cerr << "ai input D \n";
		// }
		res = this->InputHandle(aiInput);
		for (auto *j : res) {
			j->Execute(i, dt);
		}
	}
...
void Game::Render() {
	if (this->State == GAME_ACTIVE) {
		...
		Renderer->DrawSprite(
			...
			glm::vec2(0.0F, backgroundYPosition - 1.0 * this->Height),
			glm::vec2(this->Width, this->Height), 0.0F);
		// 玩家
...
void Game::DoCollisions() {
	if (Player->ultraTick < 0.01) {
		for (auto *enemy : this->Enemies) {
			...
			if (CheckCollision(GameObject(*Player), GameObject(*enemy)) !=
				0U) {
...
void Game::LoadLevel() {
	std::cerr << "Current Level: [" << this->CurrentLevel << "] - '"
			  << Levels[this->CurrentLevel] << "'\n";
	auto level = LevelLoader::GetLevel(Levels[this->CurrentLevel].c_str());
	this->Waves.clear();
	for (const auto &i : level) {
		this->Waves.emplace_back();
		std::cerr << "Wave [" << this->Waves.size() - 1 << "]: \n";
		for (const auto &j : i) {
			auto t = ResourceManager::GetTexture(j.type);
			auto size = glm::vec2(t.Width, t.Height) * PlaneSpriteScale;
			auto pos =
				glm::vec2(j.position * static_cast<float>(this->Width) * 0.8F +
							  static_cast<float>(size.x) * 0.5F +
							  static_cast<float>(this->Width) * 0.1F,
						  0.0F);
			std::cerr << "  - '" << j.type << "'x" << j.count << " at "
					  << j.position << "(" << pos.x << ", " << pos.y << ")"
					  << "\n";
			for (int k = 0; k < j.count; ++k) {
				// std::cerr << " + ";
				auto tpos =
					glm::vec2(pos.x - static_cast<float>(size.x) * 0.5F +
								  std::uniform_real_distribution<float>(
									  -size.x, size.x)(rnd),
							  pos.y - static_cast<float>(k + 1) * size.y);
				this->Waves.back().emplace_back(&this->aiSimple, tpos, size, t,
												ENEMY_COLOR,
												glm::vec2(0.0, 30.0));
			}
			// std::cerr << "\n";
...
void Game::LoadWave() {
	std::cerr << "===== Wave [" << this->CurrentWave << "] =====\n";
	this->Enemies.clear();
	for (auto &i : Waves[CurrentWave]) {
		this->Enemies.push_back(&i);
		std::cerr << "  - (" << i.Position.x << ", " << i.Position.y << ")\n";
	}
	std::cerr << "total enemies: " << this->Enemies.size() << "\n";
}
